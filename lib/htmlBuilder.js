module.exports = function(file, blocks, options, push, callback) {
  ...
  function transformOutputPath(path) {
    if (options && typeof(options.transformOutputPath) === 'function')
      return options.transformOutputPath.call(this, path);
    return path;
  }
      ...
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.js')
            html[i] += '<script src="' + transformOutputPath(name.replace(path.basename(name), path.basename(file.path))) +  '"' + createHTMLAttributes(jsAttributes, jsCounter++) +'></script>';
          resolve();
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.css') {
            html[i] += '<link rel="stylesheet" href="' + transformOutputPath(name.replace(path.basename(name), path.basename(file.path)))
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          push(file);
          html[i] += '<link rel="import" href="' + transformOutputPath(name.replace(path.basename(name), path.basename(file.path))) + '"/>';
          resolve();
        }.bind(this, block.nameInHTML));
      }
    });
