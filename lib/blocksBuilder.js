      ...
      if (pipeline !== 'remove') {
        if (pipeline === 'htmlimport'){
          blocks.push({
            ...
            tasks: options[pipeline]
          });

        } else if (pipeline.indexOf('js') !== -1) {
          if (pipeline.indexOf('inline') !== -1) {
            blocks.push({
              type: 'inlinejs',
        ...
        } else if (pipeline.indexOf('css') !== -1 || pipeline.indexOf('less') !== -1) {
          if (pipeline.indexOf('inline') !== -1) {
            blocks.push({
              type: 'inlinecss',
              files: getFiles(resourceHtmlMarkup, options.cssReg || cssReg, alternatePath),
